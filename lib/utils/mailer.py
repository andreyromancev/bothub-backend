...
from lib.utils.decorators import pass_for_development


def mail_manager(template_path, context=None):
    send_mail.delay([settings.MAILER_MANAGER_EMAIL], template_path, context)


def mail_admin(template_path, context=None):
    send_mail.delay([settings.MAILER_ADMIN_EMAIL], template_path, context)


def mail_users(email_list, template_path, context=None):
    send_mail.delay(email_list, template_path, context)
